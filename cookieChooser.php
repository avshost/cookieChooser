...
class CookieChooser {
	...
	$optionSelectString="<option></option>";

	foreach($optionsArray as $opt){
		$row=explode(":",$opt);
		$optionSelectString=$optionSelectString."<option name='$row[1]'>$row[0]</option>";		
	}
	...
	<select name="$name" id="$name" onchange="cookieChooser$name($name)">
		$optionSelectString
	<select>
OUTPUT;
	return $outputVar;
